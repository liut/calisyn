...
<template>
  <template v-if="image">
    <NAvatar v-if="isString(avatar) && avatar.length > 0" :src="avatar" :fallback-src="defaultAvatar" :style="{ with: '32px', height: '32px' }" />
    <NAvatar v-else round :src="defaultAvatar" :style="{ with: '32px', height: '32px' }" />
  </template>
  <template v-else>
    <NAvatar :src="serviceAvatar" :style="{ with: '32px', height: '32px', background: '#fff' }" />
  </template>
